from PyQt5.QtWidgets import QWidget, QMessageBox, QTableWidget, QTableWidgetItem, QHeaderView, QVBoxLayout, QPushButton
...
class Amazon(QWidget):
    ...
    def create_ps4_table(self):

        column_names = ['Name of Game', 'Price of Game']
    ...
    def see_product(self):
        ...
        cur = conn.cursor()
        try:
            query = 'SELECT * FROM ps4_tbl WHERE name_of_game=%s'
            cur.execute(query, (self.product_name, ))
            product_details = cur.fetchone()
            if product_details is not None:
                self.product_page = ProductPage(product_details, 1)
            else:
